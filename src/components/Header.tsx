...
export default function Header({ variant = "default" }: HeaderProps) {
  ...
  const hamburgerButtonClass =
    "inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200 text-slate-700 transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#2563EB] focus:ring-offset-2 md:hidden";
  ...
  const renderRightSection = () => (
    <>
      {variant === "onboarding" ? (
        ...
        <div className="flex items-center gap-2">
          <div className="h-11 w-11 shrink-0 animate-pulse rounded-xl bg-slate-200 md:hidden" aria-hidden aria-busy />
          <div className="h-11 w-[88px] shrink-0 animate-pulse rounded-xl bg-slate-200" aria-hidden aria-busy />
        </div>
      ) : isLoggedIn ? (
